...
public class CampService {
	 ...
	 * //boarddao.updateGrpstep(param); Board board = new Board(); int maxnum =
	 * boarddao.maxNum(); //등록된 게시물의 최대 num값을 리턴 int grplevel
	 * =(Integer.parseInt(param.get("grplevel"))); int grpstep =
	 * (Integer.parseInt(param.get("grpstep"))); board.setNum(++maxnum);
	 * board.setBoardid(param.get("boardid"));
	 * //board.setNum(Integer.parseInt(param.get("num")));
	 * board.setGrp(Integer.parseInt(param.get("grp")));
	 * board.setGrplevel(++grplevel); board.setGrpstep(++grpstep);
	 * board.setWriter(param.get("writer")); board.setPass(param.get("pass"));
	 * board.setTitle(param.get("title")); board.setContent(param.get("content"));
	 * boarddao.insert(board); return board; }
	 */
